...
```{block, type="alert alert-success"}
> **Question 4**: Create a subset of `Buildings` with only `Gaia` and `Aurora`, using index slicing in R and Python. Hint: You may wish to use the functions `c()` for R. To vectorize list in Python, `numpy.array()` would be useful, but creating plain list doesn't need it :)
```
...
We just saw that we can slice arrays based on conditions. In which `&` and `|` are used for both Python and R. Note that the boolean operators `&` and `|` (for *and* and *or* respectively) are valid in both R and Python, but they mean different things!
...
This is good in some cases, such as slicing arrays using boolean arrays (like in Question.5, where `&&` doesn't really work), and not so good in other cases, like `if` statements that control program flow.  

The conditionals `if` and `else` are used to structure our code, so that part of the code only runs when certain conditions are met. In this case, `if (c(TRUE, FALSE))` is impossible to determine and thus will throw an error. The form `&&` only compares one boolean with another boolean and throws an error otherwise, and thus is suitable. 
...
This behavior makes them useful in contexts like Question.5, where we compare an array of values against numeric thresholds and want elementwise boolean logic.
...
```{block, type="alert alert-success"}
> **Question 6**: Why we don't need parentheses here in this Python code? Hint: it's something to do with operator precedence.
```
